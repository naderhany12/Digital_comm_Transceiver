...
different realization of the random process. Also a random time delay is added
using \texttt{randi} function, then \texttt{circshift} is used to apply the
random delay to each realization, ensuring that each signal starts at a
different time instant.
\begin{itemize}

    \item \texttt{data} parameter is the generated random bits from
          \autoref{listing:Data-Generation}. These data will be mapped to $0$ and $A$ for
...
All we did in \autoref{listing:Unipolar-Ensemble} is setting the pulse
parameter and passing all the existing parameters to the
\texttt{build\_ensemble} function, for the data we used the generated data from
\autoref{listing:Data-Generation}, for \texttt{A}, \texttt{$N_{realizations}$},
\texttt{L} we used the defined amplitude, number of realizations, and Samples
per symbol period from the control flags in \autoref{listing:control-flags}.
...
Now we can see in \autoref{fig:UNIPOLAR-ENSEMBLE} the first two realizations of
the unipolar ensemble, the signal is a rectangular pulse as we expected from
the input pulse, and the amplitude maps to $0$ and $4$ as how we assigned A
earlier in the control flags. This output confirms that we succefully created
the unipolar ensemble as we met the required specifications for the unipolar
line code.

\begin{itemize}
    \item The amplitude of the signal is either $0$ (for bit "0") or $A$ (for bit "1").
