% Author Name: José Areia 
% Author Contact: jose.apareia@gmail.com
% Version: 2.2.13 - 2026/01/06
% Public Repository: https://github.com/joseareia/ipleiria-thesis
% Wiki/Getting Help: https://github.com/joseareia/ipleiria-thesis/wiki

%%% Document Options %%%
% !TEX program = xelatex
\documentclass[
    language=english,
    school=estg,
    docstage=final,
    media=screen,
    bookprint=false,
    linkcolor=black!45!black,
    chapterstyle=modern,
    coverstyle=classic,
    aiacknowledgement=true,
    listprefix=false
]{IPLeiriaThesis} % Refer to the Wiki for a list of available options.
%%% TikZ Setup %%%
\usepackage{tikz}
\usetikzlibrary{shapes,arrows,positioning} 
\usetikzlibrary{arrows.meta}

%%% Bidi (must be loaded after tikz, enables Arabic RTL via \RLE{...}) %%%
\usepackage{bidi}

%%% Document Version %%%
\DocumentVersion{1.0.0} % Required only if the 'docstage' is set to 'working'.

%%% Document Metadata %%%
\input{Metadata/Metadata}

%%% Loading of Glossary and Acronyms %%%
\makeglossaries
\loadglsentries{Matter/05-Glossary}
\loadglsentries[\acronymtype]{Matter/06-Acronyms}
\loadglsentries[\symboltype]{Matter/07-Symbols}

\begin{document}

%%% Front Matter %%%
\include{Matter/00-Cover}
\include{Matter/01-Front-Page}

%%% Copyright Statement %%%
%\include{Matter/02-Copyright}

%%% Roman Numeration %%%
\pagenumbering{roman}

%%% Acknowledgements %%%
%\include{Matter/03-Acknowledgements}

%%% Abstract %%%
\include{Chapters/00-Abstract}

%%% AI Acknowledgement %%%
%\include{Matter/04-AI}

%%% Table of Contents, List of Figures and List of Tables %%%
\bookmarktocentry\tableofcontents
%\listoffigures
%\listoftables

%%% Print: Glossary and Acronyms %%%
%\glossarytoc\printnormalglossary
%\acronymtoc\printacronymglossary
%\symboltoc\printsymbolglossary

%%% Arabic Numeration %%%
\pagenumbering{arabic}

%%% Chapters (**Insert Yours Here**) %%%
\include{Chapters/01-Problem-Description}
\include{Chapters/02-Introduction}
\include{Chapters/03-Control-Flags}
\include{Chapters/04-Generation-of-Data}
\include{Chapters/05-Creating-the-Unipolar-Ensemble}
\include{Chapters/06-Creating-The-Polar-NRZ-Ensemble}
\include{Chapters/07-Creating-The-Polar-RZ-Ensemble}
\include{Chapters/08-Applying-Random-Initial-Time-Shifts}
\include{Chapters/09-Preparing-Cell-Arrays-For-Analysis}
\include{Chapters/10-Q1-Statistical-Mean-Calculation}
\include{Chapters/11-Q2-Is-the-Process-Stationary}
\include{Chapters/12-Q3-Statistical-Autocorrelation}
\include{Chapters/13-Q4-Time-Mean-&-Autocorrelation-of-One-Waveform}
\include{Chapters/14-Q5-Is-the-Random-Process-Ergodic}
\include{Chapters/15-Plotting-the-PSD-of-the-Ensemble}
\include{Chapters/16-Q6-Signal Bandwidth}

%%% Appendices: Work that *YOU* Developed %%%
\appendix
\input{Matter/08-Appendices}
\input{Chapters/Appendices/00-AppendixA}
%\input{Chapters/Appendices/01-AppendixB}

%%% Bibliography %%%
\renewcommand{\refname}{Bibliography}
\printbibliography[title={\refname},heading=bibintoc]

%%% Annexes: Work that *YOU DID NOT* Develop %%%
%\input{Matter/09-Annexes}
%\input{Chapters/Annexes/00-AnnexA}
%%% Back Page %%%
%\input{Matter/10-Back-Page}

\end{document}