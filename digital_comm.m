...
ensample_unipolar = kron(Tx_Unipolar, pulse_unipolar); %upsamling
...
% 1) Subtract mean^2 from every lag to isolate the AC (sinc²) part
...
% 4) Represent the DC impulse as a single raised bin (area = mx²)
%    Scale so the spike is visually comparable to the sinc² lobe
...
% CASE 2: TIME AUTOCORRELATION
    elseif strcmp(type, 'time')
        if isvector(signal)
            signal = signal(:)'; % Ensure signal is a row vector (1 × N)
        end
        N = length(signal);
 % Loop through each lag value
        for i = 1:length(tau)
            ...
            % ================================================================
            % POSITIVE LAG (τ ≥ 0): Shift signal to the right
            % Valid indices: n = 1 to N-τ
            % ================================================================
            if lag >= 0
                for n = 1:N-lag
            ...
            % ================================================================
            % NEGATIVE LAG (τ < 0): Shift signal to the left
            % Valid indices: n = 1-τ to N (since lag is negative)
            % Equivalent to positive lag with symmetric property: R(-τ) = R(τ)
            % ================================================================
            else
